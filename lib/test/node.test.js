...
describe("constructor", () => {
    const given = getGiven();
    given("node", () => given.params === undefined ? new Node() : new Node(given.params));
...
describe("getNodeByCallback", () => {
    ...
    context("when a matching node exists", () => {
        it("returns the node", () => {
            expect(given.tree.getNodeByCallback((node) => node.name.startsWith("chi"))).toMatchObject({
                name: "child1",
            });
        });
    });
...
describe("getNodeByNameMustExist", () => {
    ...
    context("when the node doesn't exist", () => {
        it("throws an exception", () => {
            expect(() => given.tree.getNodeByNameMustExist("non-existing")).toThrow("Node with name non-existing not found");
        });
    });
});
describe("getParent", () => {
...
describe("iterate", () => {
    ...
    context("when the visitor function returns true", () => {
        const visitAllNodes = (node, level) => {
...
describe("moveNode", () => {
    ...
    context("when moving after a node", () => {
        it("moves the node", () => {
            expect(given.tree.moveNode(given.child2, given.node2, Position.After)).toBe(true);
            expect(given.tree).toMatchObject({
                name: "",
                children: [
                    expect.objectContaining({
        ...
        context("when the target is the root node", () => {
            it("returns false", () => {
                expect(given.tree.moveNode(given.child2, given.tree, Position.After)).toBe(false);
            });
        });
    });
    context("when moving inside a node", () => {
        it("moves the node", () => {
            expect(given.tree.moveNode(given.child1, given.node2, Position.Inside)).toBe(true);
            expect(given.tree).toMatchObject({
                name: "",
                children: [
                    expect.objectContaining({
    ...
    context("when moving before a node", () => {
        it("moves the node", () => {
            expect(given.tree.moveNode(given.child2, given.child1, Position.Before)).toBe(true);
            expect(given.tree).toMatchObject({
                name: "",
                children: [
                    expect.objectContaining({
        ...
        context("when the target is the root node", () => {
            it("returns false", () => {
                expect(given.tree.moveNode(given.child2, given.tree, Position.Before)).toBe(false);
            });
        });
    });
    context("when the moved node is a parent of the target node", () => {
        it("doesn't move the node", () => {
            expect(given.tree.moveNode(given.node1, given.child1, Position.Before)).toBe(false);
            expect(given.tree).toMatchObject({
                children: [
                    expect.objectContaining({
                        name: "node1",
    ...
    context("with position None", () => {
        it("returns false", () => {
            expect(given.tree.moveNode(given.child2, given.node2, Position.None)).toBe(false);
        });
    });
});
describe("prepend", () => {
