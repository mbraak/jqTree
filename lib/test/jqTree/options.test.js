...
describe("dataUrl", () => {
  const exampleStructure = [expect.objectContaining({
    name: "node1"
  }), expect.objectContaining({
    name: "node2"
  })];
  ...
  beforeEach(() => {
    server.use(_msw.rest.get("/tree/", (request, response, ctx) => {
      ...
      const data = nodeName ? [nodeName] : _exampleData.default;
  ...
  testCases.forEach(_ref => {
    ...
    context(`with ${name}`, () => {
      it("loads the data from the url", async () => {
        ...
        await _dom.screen.findByText(expectedNode);
...
describe("onCreateLi", () => {
  ...
  beforeEach(() => {
    given.$tree.tree({
      data: _exampleData.default,
      onCreateLi: (node, el) => {
        (0, _testUtil.titleSpan)(el).text(`_${node.name}_`);
      }
    });
  });
...
describe("onLoadFailed", () => {
  ...
  context("when the loading fails", () => {
    beforeEach(() => {
      server.use(_msw.rest.get("/tree/", (_request, response, ctx) => response(ctx.status(500), ctx.body("Internal server error"))));
    });
