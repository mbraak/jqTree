...
export default class ElementsRenderer {
    ...
    createDomElements(element, children: Array<Node>, is_root_node: boolean, is_open: boolean, level: number) {
    ...
    attachNodeData(node: Node, li) {
        node.element = li;
        $(li).data('node', node);
    }

    createUl(is_root_node: boolean) {
        let class_string;
        let role;
    ...
    createLi(node: Node, level: number) {
        const is_selected = this.tree_widget.select_node_handler && this.tree_widget.select_node_handler.isNodeSelected(node);
        let li;
    ...
    createFolderLi(node: Node, level: number, is_selected: boolean) {
    ...
    createNodeLi(node: Node, level: number, is_selected: boolean) {
        ...
        // title span
        div.appendChild(
            this.createTitleSpan(node.name, level, is_selected, node.is_open, false)
        );

        return li;
    }

    createTitleSpan(node_name: string, level: number, is_selected: boolean, is_open: boolean, is_folder: boolean) {
    ...
    getFolderClasses(node: Node, is_selected: boolean) {
    ...
    escapeIfNecessary(value: string): string {
